  ...
  return (
    <div className='shadow p-5 mb-3 flex gap-10 items-center'>
      ...
      <div className='md:w-4/6'>
        <p className='text-3xl font-bold'>{producto.productname}</p>
        <p className='text-xl font-bold mt-2'>Cantidad: {producto.cantidad}</p>
        <p className='text-xl font-bold text-amber-500 mt-2'>
          Precio: {formatearDinero(producto.price)}
        </p>

        <p className='text-sm text-gray-700 mt-2'>
          Subtotal: {formatearDinero(producto.price * producto.cantidad)}
        </p>
      </div>
      ...
      <div>
        <button
          ...
          onClick={() => handleEditarCantidades(producto._id)}
        >
          <svg
